...
- `core/` — NÚCLEO COMPARTIDO: `config.py` (settings desde `.env`), `database.py` (las 3 conexiones y `Base` del ORM), `models.py` (`Usuario`, `RolUsuario`, `UsuarioModulo`), `auth.py` (login/me/logout, `get_usuario_actual`), `permisos.py` (`MODULOS`, `ROLES_MODULO`, `modulos_de`, `tiene_permiso`, `requiere_modulo`), `modulos.py` (`ModuloInfo`: clave, nombre, descripcion, activo, routers, etiquetas_rol, panel_gerencial — ADR-0013), `asistente.py` (chat de ayuda, transversal), `quincena.py` (`calcular_rango_quincena`).
- `modulos/__init__.py` — el registro de módulos del sistema: `REGISTRO = (PRELIQUIDACION, TERCEROS)`, `activos()`, `claves()`. Agregar un módulo nuevo es una línea acá.
- `modulos/preliquidacion/` — el módulo activo hoy: `__init__.py` (arma su `ModuloInfo`, expone `routers`), `api/` (`preliquidacion.py`, `precios.py`, `export.py`, `gerencial.py`; autorización por rol de módulo con `requiere_modulo` — ver CONTEXT.md, "Rol"), `permisos.py` (`requiere_operativo`, `requiere_conceptos`, `requiere_gerencial`, sobre `requiere_modulo` del núcleo), `services/` (`preliquidacion_service.py` el motor: matching de conceptos, recálculo reactivo, controles; `motor_reglas.py` cálculo de cantidad por unidad, empresa/legajo, duplicados; `gerencial_service.py` KPIs de mano de obra; `consulta_externa.py` extracción de tareas de la base de campo; `sueldos_service.py` resolución de empleados contra sueldos; `export_service.py`; `solapamiento_service.py`), `models.py` (modelos del módulo, reexporta `Usuario` del núcleo), `schemas.py` (DTOs Pydantic).
- `modulos/terceros/` — el molde del segundo módulo (PR 4 de la etapa 0, ADR-0013), registrado pero **inactivo** (`activo=False` en su `ModuloInfo`; ver "Módulo activo" en `CONTEXT.md`): `__init__.py`, `permisos.py`, `api/terceros.py` (un endpoint de estado), `models.py`, `schemas.py`, `services/`, `consulta_externa.py`. Se completa siguiendo `docs/modulos/GUIA-MODULOS.md`.
- `tests/` — pytest (sqlite in-memory), separados en `tests/core/` (autorización por módulo, arranque), `tests/preliquidacion/` (el resto, 22 archivos) y `tests/terceros/` (molde, `test_molde.py`). Corren con `python -m pytest -q`.
- `migrations/core/` — SQL manual del núcleo (ver §3): `000_usuarios.sql`, `001_usuario_modulo.sql`. `migrations/preliquidacion/` — SQL manual del módulo (ver §3): `000_esquema_base.sql` + `ws1`…`ws16` (14 archivos; no existen `ws4` ni `ws6`) + `fix_trazabilidad_concepto_adicional.sql`. `migrations/terceros/` — molde, inactivo (`LEEME.md`; la primera migración real será `001_crear_tablas.sql`). `docs/adr/` — decisiones. `CONTEXT.md` — glosario.

La guía para agregar un módulo nuevo (empezando por Liquidación Terceros) está en `docs/modulos/GUIA-MODULOS.md`; el glosario de ese módulo, en `docs/modulos/terceros/CONTEXT-terceros.md`, y su plan de implementación en `docs/modulos/terceros/plan-terceros.md`.
