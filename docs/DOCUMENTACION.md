...
- `modulos/__init__.py` — el registro de módulos del sistema: `REGISTRO = (PRELIQUIDACION, FLETES)`, `activos()`, `claves()`. Agregar un módulo nuevo es una línea acá.
...
- `modulos/fletes/` — el molde del segundo módulo (PR 4 de la etapa 0, ADR-0013), registrado pero **inactivo** (`activo=False` en su `ModuloInfo`; ver "Módulo activo" en `CONTEXT.md`): `__init__.py`, `permisos.py`, `api/fletes.py` (un endpoint de estado), `models.py`, `schemas.py`, `services/`, `consulta_externa.py`. Se completa siguiendo `docs/modulos/GUIA-MODULOS.md`.
- `tests/` — pytest (sqlite in-memory), separados en `tests/core/` (autorización por módulo, arranque), `tests/preliquidacion/` (el resto, 22 archivos) y `tests/fletes/` (molde, `test_molde.py`). Corren con `python -m pytest -q`.
- `migrations/core/` — SQL manual del núcleo (ver §3): `000_usuarios.sql`, `001_usuario_modulo.sql`. `migrations/preliquidacion/` — SQL manual del módulo (ver §3): `000_esquema_base.sql` + `ws1`…`ws16` (14 archivos; no existen `ws4` ni `ws6`) + `fix_trazabilidad_concepto_adicional.sql`. `migrations/fletes/` — molde, inactivo (`LEEME.md`; la primera migración real será `001_crear_tablas.sql`). `docs/adr/` — decisiones. `CONTEXT.md` — glosario.

La guía para agregar un módulo nuevo (empezando por Fletes) está en `docs/modulos/GUIA-MODULOS.md`; el glosario del dominio de Fletes, en `docs/modulos/fletes/CONTEXT-fletes.md`.
